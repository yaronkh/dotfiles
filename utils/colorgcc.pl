...
while(<GCCOUT>)
{
  ...
  elsif (m/^(.*):( *warning):(.*)/) # warning
  {
    ...
    # Warning
    print($colors{"warningFileNameColor"}, "$field1:", color("reset"));
    print($colors{"warningMessageColor"}, "$field2:", color("reset"));
    srcscan($field3, $colors{"warningMessageColor"});
    print("\n");
  }
  elsif (m/^(.*):( *NOTE):(.*)/) # note
  ...
  {
          # This is a gcc command line, so print it in the intro color.
          print($colors{"introColor"}, $_, color("reset"));
  }
  elsif (m/(CC +)(\[.*\] +)(.*)$/ || m/(LD +)(\[.*\] +)(.*)$/)
  {
          my $field1 = $1 || "";
          ...
          # This is a gcc command line, so print it in the intro color.
          print($colors{"introFileNameColor"}, "$field1", color("reset"));
          print($colors{"introColor"}, "$field2", color("reset"));
          print($colors{"introMessageColor"}, "$field3", color("reset"));
          print("\n");
  }
  elsif (m/^(make .*)$/) # gcc)
  {
          # This is a gcc command line, so print it in the intro color.
          print($colors{"introFileNameColor"}, $_, color("reset"));
  }
  else # Anything else.
  {
