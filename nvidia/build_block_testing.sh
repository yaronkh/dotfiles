...
build() {
	...
	full_make_cmd="compiledb -o compile_commands.json $MAKE_CMD write_compilation_cmd COMMIT_ID=$COMMIT_ID VER_TAGID=$VER_TAGID BRANCH_NAME=$BRANCH_NAME -j 10 $PARAM_FOR_MAKE ${@}";
	...
	full_make_cmd="compiledb -o compile_commands.json $MAKE_CMD all COMMIT_ID=$COMMIT_ID VER_TAGID=$VER_TAGID BRANCH_NAME=$BRANCH_NAME -j 10 $PARAM_FOR_MAKE ${@}";
	echo -e "|\e[0;32m$full_make_cmd\e[0;0m|";
	(cd $TEST_SUB_DIR && ${full_make_cmd})
	ret_code=$?
	if [ $ret_code != 0 ]; then
...
case "$1" in
	...
	"nightly")
		...
		execute_unitest
		runcmd="./blk_unitest -debug_dump_funcs $NIGHTLY_FLAGS $TRACES_FLAGS 2>&1 | tee out.txt"
		execute_unitest -debug_dump_funcs
		clean
		build ${@}
	...
	"ci")
		...
		clean
		build ${@} USE_RELEASE=1
		extra_args=("" "-debug_dump_funcs")
		for e_arg in "${extra_args[@]}";do
